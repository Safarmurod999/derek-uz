...
const AccordionItem = ({ item }) => {
    ...
    return (
        <li className={`accordion__item ${active ? 'active' : ''}`} onClick={() => setActive(!active)}>
            <div className='accordion__item--question' >
                <p className='accordion__button'>{lang == 'ru' ? item.question : item[`question_${lang}`]}</p>
            ...
            <div className={`accordion__item--answer`}>
                <p className='accordion__content'>{lang == 'ru' ? item.answer : item[`answer_${lang}`]}</p>
            </div>
        </li>
    )
}
